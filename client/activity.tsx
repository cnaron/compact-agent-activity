...
import { getDisplayMode, setDisplayMode } from "./transform";
...
function FoldedActivityGroup({
  ...
  const group = useSyncExternalStore(
    useCallback((notify) => subscribeGroup(turnKey, notify), [turnKey]),
    useCallback(() => getTurnGroup(turnKey), [turnKey]),
  );
  ...
  return (
    <View ref={containerRef} testID="folded-activity-group" style={containerStyle}>
      <Pressable
        ref={headerRef}
        accessibilityRole="button"
      ...
      {expanded && group ? (
        <View style={detailsContainerStyle}>
          {group.items.map((item, idx) => {
            if (item.type === "reasoning" && item.reasoningData?.text?.trim()) {
              return (
                <ReasoningText
                  key={item.id || `reasoning_${idx}`}
                  text={item.reasoningData.text}
                  phase={item.reasoningData.phase}
                  theme={theme}
                  styles={styles}
                />
              );
            }
            if (item.type === "tool_call" && item.toolCallData) {
              return (
                <GroupSubItem
                  key={item.id || `tool_${idx}`}
                  subItem={item}
                  theme={theme}
                  palette={palette}
                  styles={styles}
...
export function ColorfulReasoning({
  ...
  const settings = useSettings(activitySettings);
  const displayMode = settings.status === "ready" ? settings.values.displayMode : "folded";
  ...
  const turnKey =
    item.data.turnId ??
    (item.data.turnIndex !== undefined ? `turn_${item.data.turnIndex}` : `turn_${agentId}`);
